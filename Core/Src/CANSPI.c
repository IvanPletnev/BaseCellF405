...
bool CANSPI_Initialize(void) {
	...
	RXF2reg.RXF2SIDH = 0xA8;
	RXF2reg.RXF2SIDL = 0x20;
	RXF2reg.RXF2EID8 = 0x00;
	RXF2reg.RXF2EID0 = 0x00;

	RXF3reg.RXF3SIDH = 0xAA;
	RXF3reg.RXF3SIDL = 0x60;
	RXF3reg.RXF3EID8 = 0x00;
	RXF3reg.RXF3EID0 = 0x00;

	RXF4reg.RXF4SIDH = 0x4C;
	RXF4reg.RXF4SIDL = 0x00;
	RXF4reg.RXF4EID8 = 0x00;
	RXF4reg.RXF4EID0 = 0x00;

	RXF5reg.RXF5SIDH = 0x44;
	...
	MCP2515_WriteByte(MCP2515_RXB0CTRL, 0x24); //Enable BUKT, Standard identifiers filtered
	MCP2515_WriteByte(MCP2515_RXB1CTRL, 0x20);   //Standard identifiers filtered
	MCP2515_WriteByte(MCP2515_CANINTE, 0x03);//Receive buf 0 & 1 interrupts enable
...
uint8_t CANSPI_Receive(uCAN_MSG *tempCanMsg) {
	...
	/* Проверить, есть ли полученные сообщения в буфере */
	if (rxStatus.rxBuffer != 0) {
		/* Проверить, в каком буфере находится сообщение, и обработать его */
		if ((rxStatus.rxBuffer == MSG_IN_RXB0) || (rxStatus.rxBuffer == MSG_IN_BOTH_BUFFERS)) {

			MCP2515_BitModify(MCP2515_CANINTF, 0x01, 0);
			MCP2515_ReadRxSequence(MCP2515_READ_RXB0SIDH, rxReg.rx_reg_array, sizeof(rxReg.rx_reg_array));

		} else if (rxStatus.rxBuffer == MSG_IN_RXB1) {

			MCP2515_BitModify(MCP2515_CANINTF, 0x02, 0);
